...
def register_io_callbacks(app: dash.Dash, ctx: AppConfig) -> None:
    # ... [Keep section 1, 2, 3, and 4 exactly as they were] ...

    # [Including relevant parts for section 1-4 for context, though they are unchanged]
    @app.callback(
    ...
    def update_saved_figure_dropdown(session_data, current_value):
        ...
        for fig in session.figures:
            display = f"{fig.label or fig.view_id} ({fig.dataset_key})"
    ...
    def load_figure_from_dropdown(n_clicks, figure_id, session_data):
        ...
        if not meta: raise exceptions.PreventUpdate
    ...
    def save_current_figure(n_clicks, fs_data, figure_label, session_data, active_session_id, active_figure_id,
                            current_selection):
        if not n_clicks or not fs_data: raise exceptions.PreventUpdate
        ...
        try:
            ...
            session = ctx.session_service.ensure_session(session_from_dict(session_data), session_id=active_session_id)
            session, saved_id, is_overwrite = ctx.session_service.save_figure(
                session, active_figure_id=effective_id, dataset_key=getattr(ds, "key", ds.name),
                view_id=state.view_id, filter_state=state.to_dict(), label=figure_label,
            )
            status = f"{'Updated' if is_overwrite else 'Saved'} '{figure_label or state.view_id}' to report"
            return session_to_dict(session), active_session_id, status, (saved_id if is_overwrite else None)
        except Exception as e:
            logger.error("Save failed: %s", str(e))
            return dash.no_update, dash.no_update, "Internal Save Error", dash.no_update

    # ---------------------------------------------------------
    # 5. Export Logic (FIXED for DatasetSummary)
    # ---------------------------------------------------------
    @app.callback(
    ...
    def download_current_data(n_clicks, fs_data):
        ...
        logger.warning("View %s returned data type %s which is not supported for download.", state.view_id, type(data))
        raise exceptions.PreventUpdate